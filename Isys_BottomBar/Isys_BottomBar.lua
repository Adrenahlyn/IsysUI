-----------------------------------------------------------------------------------------------
-- Client Lua Script for Isys_BottomBar
...
function Isys_BottomBar:Init()
    Apollo.RegisterAddon(self)
end

function Isys_BottomBar:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("Isys_BottomBar.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self) 
	Apollo.LoadSprites("../Isys_Library/Isys.xml", "Isys")
end

function Isys_BottomBar:OnDocumentReady()
...
function Isys_BottomBar:RedrawCooldown()
	Apollo.StopTimer("BaseBarCorner_RedrawCooldown")
	self.bOnRedrawCooldown = false
	self:RedrawAllPastCooldown()
end

function Isys_BottomBar:RedrawAll()
...
function Isys_BottomBar:RedrawAllPastCooldown()
...
function Isys_BottomBar:RedrawPathXP()
...
function Isys_BottomBar:ConfigurePathXPTooltip(unitPlayer)
	if not PlayerPathLib then
		return ""
	end
...
function Isys_BottomBar:RedrawXP()
...
function Isys_BottomBar:RedrawEP()
...
function Isys_BottomBar:ConfigureEPTooltip(unitPlayer)
	local nCurrentEP = GetElderPoints()
...
function Isys_BottomBar:ConfigureXPTooltip(unitPlayer)
	local nCurrentXP = GetXp() - GetXpToCurrentLevel() 		-- current amount of xp into the current level
	local nNeededXP = GetXpToNextLevel() 					-- total amount needed to move through current level
	local nRestedXP = GetRestXp() 							-- amount of rested xp
	local nRestedXPPool = GetRestXpKillCreaturePool() 		-- amount of rested xp remaining from creature kills
...
function Isys_BottomBar:OnEnteredCombat(unitArg, bInCombat)
...
function Isys_BottomBar:OnClearCombatFlag()
	self.bInCombat = false
	self:RedrawAll()
end

function Isys_BottomBar:OnXpChanged()
	if GetXp() == 0 then
		return
	end
...
function Isys_BottomBar:OnPathClicked()
	Event_FireGenericEvent("PlayerPathShow")
end

function Isys_BottomBar:OnXpClicked()
	Event_FireGenericEvent("ToggleCharacterWindow")
end

function Isys_BottomBar:OnUpdateInventory()
	local unitPlayer = GameLib.GetPlayerUnit()
	if not unitPlayer then
		return
	end
	...
	if nOccupiedInventory == nTotalInventory then
		...
		self.wndInvokeButton:ChangeArt("Isys:InventoryButton")
...
function Isys_BottomBar:OnToggleFromDatachronIcon()
...
---------------------------------------------------------------------------------------------------
function Isys_BottomBar:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
...
local BaseBarCornerInst = Isys_BottomBar:new()
